...
이 프로젝트는 단순히 LLM API를 호출하는 수준이 아니라,  
운영 장애 도메인에 맞춰 RAG 구조를 직접 설계하고 구현한 프로젝트입니다.

특히 다음 부분을 강조할 수 있습니다.

1. 운영 장애 보고서 작성이라는 실제 백엔드 운영 문제를 AI로 해결하려고 했다.
2. 과거 장애 문서를 chunking, embedding, Vector DB 저장까지 구현했다.
3. 현재 장애와 유사한 과거 장애를 검색하는 semantic search를 구현했다.
4. 검색된 과거 장애 chunk를 prompt에 포함해 RAG 기반 분석을 수행했다.
5. 응답에 sources를 포함해 LLM 분석의 근거를 추적 가능하게 했다.
6. LLM이 원인을 단정하지 않도록 prompt에서 suspected_causes와 needs_confirmation을 분리했다.
7. 운영 장애 데이터가 늘어나도 Vector DB 기반으로 유사 사례 검색을 확장할 수 있는 구조로 설계했다.
...
OpenAI API와 ChromaDB 기반 RAG 구조를 활용해 운영 장애 보고서 자동 요약 및 유사 장애 사례 기반 분석 서비스를 구현했습니다.

### 상세 설명

FastAPI, OpenAI Embedding, ChromaDB를 활용하여 과거 장애 보고서를 chunk 단위로 색인하고,  
현재 장애 발생 시 유사 장애 사례를 검색해 LLM 분석 prompt에 반영하는 RAG 기반 장애 분석 API를 구현했습니다.

### 면접용 30초 설명

제가 만든 프로젝트는 운영 장애 보고서 자동 요약 서비스입니다.

처음에는 현재 장애 내용을 입력하면 LLM이 장애 개요, 영향 범위, 원인 후보, 조치 내역을 JSON 형태로 정리하는 기능을 만들었습니다.

이후에는 RAG 구조를 추가했습니다.  
과거 장애 보고서를 섹션 단위로 chunking하고, 각 chunk를 embedding해서 ChromaDB에 저장했습니다.  
현재 장애가 들어오면 해당 내용을 embedding한 뒤 ChromaDB에서 유사한 과거 장애 chunk를 검색하고, 검색 결과를 prompt에 포함해 LLM이 근거 기반 분석을 하도록 구성했습니다.

응답에는 sources를 포함해서 어떤 과거 장애 사례를 참고했는지 추적할 수 있게 했습니다.

---

## 14. 향후 개선 방향

- Slack / Jira / Confluence 장애 문서 자동 수집
- PDF / Markdown 장애 보고서 업로드 기반 indexing
- token 기준 chunking 및 overlap 적용
- RAG 검색 결과 reranking
- service, severity, date 기반 metadata filtering
- 운영 로그 기반 장애 원인 후보 추천
- 사용자 인증 및 관리자용 장애 문서 관리 기능
- Docker 기반 실행 환경 구성
- 테스트 코드 작성
- 배포 환경 구성
- 대용량 장애 문서 indexing batch 구성
- Vector DB 교체 가능 구조 적용
- 운영 환경에서의 비용 모니터링 및 요청 제한 적용
